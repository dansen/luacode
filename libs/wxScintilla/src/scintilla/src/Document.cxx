...
bool Document::isUnicode(int pos)
{
	if (pos <= 0 || pos >= Length()){
		return false;
	}
	unsigned int c = CharAt(pos);
...
bool Document::isUnicodeChar(char ch){
	unsigned int c = ch;
...
void Document::DelCharBack(int pos) {
	if (pos <= 0) {
		...
		int dt = isUnicode(pos - 1) ? 2 : 1;
		DeleteChars(pos - dt, dt);
	}
}
