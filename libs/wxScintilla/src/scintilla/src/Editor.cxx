...
void Editor::SetSelection(int currentPos_, int anchor_) {
	//对currentPos_减1，对anchor加1
	int dt = currentPos_ > anchor ? 1 : -1;
	if(needRefix(currentPos_)){
		currentPos_ = currentPos_ + dt;
	}
	if(needRefix(anchor_)){
		anchor_ = anchor_ + dt;
	}
	//
...
bool Editor::needRefix(int pos)
{
	if(pos <= 0 || pos > pdoc->Length()){
		return false;
	}
	//得到该位置的字符
	int cnt = 0;
	unsigned int c = pdoc->CharAt(pos);
	//计算偶数位
	while (c > 128){
		--pos;
		if (pos < 0){
...
int Editor::KeyCommand(unsigned int iMessage) {
	...
	case SCI_CHARLEFT:
		if (SelectionEmpty() || moveExtendsSelection) {
			int dt = pdoc->isUnicode(currentPos - 1) ? -2 : -1;
			MovePositionTo(MovePositionSoVisible(currentPos + dt, -1));
		} else {
	...
	case SCI_CHARRIGHT:
		if (SelectionEmpty() || moveExtendsSelection) {
			int dt = pdoc->isUnicode(currentPos) ? 2 : 1;
			MovePositionTo(MovePositionSoVisible(currentPos + dt, 1));
		} else {
