...
void Unescape(const wxString& src, wxString& dst)
{
    ...
    for (size_t i = 0; i < src.Length(); ++i)
    {
        ...
        if (src[i] == '&' && (i + 1 < src.Length()) && src[i + 1] == '#')
        {
			...
			if (wxSscanf(src.c_str() + i + 2, wxT("%d;%n"), &code, &length) == 1)
            {
                dst += (char)code;
                i += length + 1; 
            }
...
{
    return WriteXmlNode(tag, wxString::Format(wxT("%d"), data));
}
...
wxXmlNode* WriteXmlNode(const wxString& tag, const wxColour& color)
{
    ...
    return WriteXmlNode(tag, wxString::Format(wxT("#%06x"), value));
}
